...
いずれも**判定不能なら通す**（`HERDR_PANE_ID` 未設定、フォーカス未観測）。`pane.focused` を取りこぼした場合の最悪ケースが「1 キー分だけ従来挙動」で済み、「キーボードが効かないペイン」にはならない側に倒す。同じ理由で、`pane.list` スナップショット（Subscriber の再接続時・Reconciler の 60 秒スイープ）は「**自ペインがフォーカス中**」という向きにだけ採用する: L 接続断中の `pane.focused` 取りこぼしはこれで復旧できる一方、スナップショットは常にソケット 1 往復ぶん古く（issue #12）、「他ペインがフォーカス中」を信じると適用済みの新しい `pane.focused` を巻き戻して次のフォーカス変更までキーボードを殺しうる。逆向きの誤りは「キーを通す」だけで、そもそも非フォーカスのペインにキーは届かない。`a` のリフレッシュは 3 経路で最も古いのでどちら向きにも採用しない。なおこの復旧は**認識の訂正のみで窓は張らない**: スナップショットはフォーカス獲得イベントではなく、報告された獲得がいつのものか分からない上、それに伴って届いた迷子キーは pane.list が帰ってくるずっと前に stdin から読まれ処理済みである。ここで窓を張っても守るものがなく、しばらく office を見ていたユーザーの意図的な 1 キーを落とすだけになる。加えて `run()` は起動時にも窓を張る: office が未起動のときの導線は `plugin.pane.open` なので、迷子キーは stdin を読む前に pty へバッファされ、窓を張るはずの `pane.focused` は Subscriber 接続前に発火して観測できない。どちらの経路でも「まだ何も描いていない」ことは共通なので、そこを根拠にする。
